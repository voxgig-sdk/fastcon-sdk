...
Fastcon API client, generated from the OpenAPI spec.

The Fastcon API provides V2Ray-based VLESS and Trojan proxy configurations specifically designed for bypassing internet censorship, particularly for Telegram.

Learn more about Fastcon API at [github.com/l0v3m0n3y/fastcon](https://github.com/l0v3m0n3y/fastcon).

This is an unofficial SDK for the Fastcon public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — Ping and Proxy — that you
...
```go
client := sdk.Test()
result, err := client.Proxy(nil).List(
    nil, nil,
)
```
...
```bash
cd go-mcp && go build -o fastcon-mcp .
```
...
```json
{
  "mcpServers": {
    "fastcon": {
      "command": "/abs/path/to/fastcon-mcp"
    }
  }
}
```
...
| --- | --- | --- |
| **Ping** | The Ping entity (create). | `/api/ping` |
| **Proxy** | The Proxy entity (list). | `/api/proxies` |
...
```go
import sdk "github.com/voxgig-sdk/fastcon-sdk/go"
...
- Upstream API: [https://github.com/l0v3m0n3y/fastcon](https://github.com/l0v3m0n3y/fastcon)
...
Generated from the Fastcon API OpenAPI spec by [@voxgig/sdkgen](https://github.com/voxgig/sdkgen).
