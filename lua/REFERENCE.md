# Fastcon Lua SDK Reference

Complete API reference for the Fastcon Lua SDK.
...
```lua
local result, err = client:Ping():create({
  time = --[[ number ]],
})
```
...
```lua
local results, err = client:Proxy():list()
```
