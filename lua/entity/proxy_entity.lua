...
local ProxyEntity = {}
ProxyEntity.__index = ProxyEntity


function ProxyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ProxyEntity)
  self._name = "proxy"
  self._client = client
...
function ProxyEntity:get_name()
  return self._name
end


function ProxyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ProxyEntity.new(self._client, opts)
end
...
function ProxyEntity:mark_deleted()
  self._deleted = true
end


function ProxyEntity:deleted()
  return true == self._deleted
end


function ProxyEntity:data_set(args)
  if args ~= nil then
...
function ProxyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ProxyEntity:match_set(args)
  if args ~= nil then
...
function ProxyEntity:match_get()
...
function ProxyEntity:stream(action, args, callopts)
...
---@param reqmatch ProxyListMatch
---@param ctrl? table
---@return Proxy[]
---@return string? err
function ProxyEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function ProxyEntity:_run_op(ctx, post_done)
  local utility = self._utility
