...
local PingEntity = {}
PingEntity.__index = PingEntity


function PingEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PingEntity)
  self._name = "ping"
  self._client = client
...
function PingEntity:get_name()
  return self._name
end


function PingEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PingEntity.new(self._client, opts)
end
...
function PingEntity:mark_deleted()
  self._deleted = true
end


function PingEntity:deleted()
  return true == self._deleted
end


function PingEntity:data_set(args)
  if args ~= nil then
...
function PingEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PingEntity:match_set(args)
  if args ~= nil then
...
function PingEntity:stream(action, args, callopts)
...
---@param reqdata PingCreateData
---@param ctrl? table
---@return Ping
---@return string? err
function PingEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function PingEntity:_run_op(ctx, post_done)
  local utility = self._utility
