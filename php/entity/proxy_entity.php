...
class ProxyEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ProxyEntity($this->_client, $opts);
    }

    /**
     * @param Proxy|array $args Proxy data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = FastconHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Proxy|array The current Proxy data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Proxy fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = FastconHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Proxy fields).
     */
    ...
    /**
     * List Proxy items matching the given filter.
     *
     * @param ProxyListMatch|array|null $reqmatch Match filter (any subset
     *   of Proxy fields) as an assoc-array; ProxyListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Proxy[]|array A list of Proxy items as assoc-arrays at
     *   the SDK boundary; throws FastconError on failure (item-5 convention).
     */
