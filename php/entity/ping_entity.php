...
class PingEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new PingEntity($this->_client, $opts);
    }

    /**
     * @param Ping|array $args Ping data (assoc-array) to store.
     */
    ...
    /**
     * @return Ping|array The current Ping data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Ping fields).
     */
    ...
    /**
     * @return array The current match filter (any subset of Ping fields).
     */
     ...
     * Create a new Ping.
     *
     * @param PingCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed PingCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Ping|array The created Ping as an assoc-array at the
     *   SDK boundary; throws FastconError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = FastconHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
