...
class FastconSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = FastconHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = FastconHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!FastconFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, FastconFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return FastconUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = FastconHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = FastconHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = FastconHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new FastconError($op . "_allow",
                "FastconSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = FastconHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new FastconError("graphql_error",
                "FastconSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    // Canonical facade: $client->Proxy()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->proxy()
    // resolves here too.
    public function Proxy($data = null)
    {
        require_once __DIR__ . '/entity/proxy_entity.php';
        if ($data === null) {
            if ($this->_proxy === null) {
                $this->_proxy = new ProxyEntity($this, null);
            }
            return $this->_proxy;
        }
        return new ProxyEntity($this, $data);
    }
