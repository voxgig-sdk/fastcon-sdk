...
class FastconNetsimFeature extends FastconBaseFeature
{
    ...
    private function _simulate(FastconContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(FastconContext $ctx, array $applied): void
    {
        $client = $this->client;
