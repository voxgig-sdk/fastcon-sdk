...
// Fastcon SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = FastconOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(FastconOmni::structfix($now, Struct::undef()))
                        !== json_encode(FastconOmni::structfix($val, Struct::undef()))) {
