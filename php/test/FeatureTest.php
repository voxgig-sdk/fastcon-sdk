...
class FtCtrl extends FastconControl
{
...
class FtClient
{
    ...
    private FastconUtility $utility;

    public function __construct(FastconUtility $utility, array $options)
    {
    ...
    public function get_utility(): FastconUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FastconContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FastconContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FastconConfig::make_feature($name);
    ...
    private static function buildUrl(FastconSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FastconError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FastconResponse($fetched);
        }
    ...
    private function fail(FastconContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FastconContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
