...
{
    private static function utility(): FastconUtility
    {
        return new FastconUtility();
    }
    ...
    private static function ctx(array $over = []): FastconContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new FastconContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new FastconOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof FastconError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new FastconOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = FastconMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new FastconOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = FastconMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new FastconOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = FastconMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FastconSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = FastconMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new FastconResponse([]),
            'result' => new FastconResult([]),
        ]);
        [, $err] = FastconMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new FastconSpec([]),
            'response' => null,
            'result' => new FastconResult([]),
        ]);
        [, $err] = FastconMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new FastconSpec(['step' => 's']),
            'response' => new FastconResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new FastconResult([]),
        ]);
        [, $err] = FastconMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new FastconSpec(['step' => 's']),
            'response' => new FastconResponse(self::resp(200, ['v' => 1])),
            'result' => new FastconResult([]),
        ]);
        [, $err] = FastconMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new FastconSpec(['step' => 's']),
            'response' => new FastconResponse(self::resp(200, ['v' => 2])),
            'result' => new FastconResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        FastconMakeResponse::call($ctx);
    ...
    {
        $preset = new FastconResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new FastconSpec([]),
            'response' => new FastconResponse([]),
            'result' => new FastconResult([]),
        ...
        [$got, $err] = FastconMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new FastconResult([])]);
        [, $err] = FastconMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new FastconSpec([]), 'result' => null]);
        [, $err] = FastconMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FastconSpec(['step' => 's']),
            'result' => new FastconResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new FastconOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FastconMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FastconSpec(['step' => 's']),
            'result' => new FastconResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new FastconOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FastconMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FastconResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new FastconSpec([]),
            'result' => new FastconResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = FastconMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FastconMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new FastconError('boom', 'boom');
        $utility->fetcher = function (FastconContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FastconSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = FastconMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (FastconContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = FastconMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(FastconResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (FastconContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FastconSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = FastconMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new FastconResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new FastconSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = FastconMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = FastconMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = FastconMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new FastconResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, FastconDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new FastconResult(['ok' => false])]);
        $this->expectException(FastconError::class);
        FastconDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new FastconResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', FastconMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        FastconMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new FastconBaseFeature();
        $b = new FastconBaseFeature();
        FastconFeatureAdd::call($ctx, $a);
        FastconFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): FastconBaseFeature
    {
        $f = new FastconBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        FastconFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): FastconContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new FastconSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FastconPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new FastconResponse(['headers' => null]),
            'result' => new FastconResult([]),
        ]);
        FastconResultHeaders::call($ctx);
