...
class FastconMakeError
{
    public static function call(?FastconContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new FastconContext([], null);
        }
        $op = $ctx->op ?? new FastconOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new FastconResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof FastconError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new FastconError('', $msg, $ctx);
        ...
        if ($err instanceof FastconError) {
