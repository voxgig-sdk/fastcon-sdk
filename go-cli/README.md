...
boru-driven command-line client **and** interactive REPL for the Fastcon
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/fastcon-cli)
make build
...
FASTCON_BASE=https://api.example.com ./fastcon-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/fastcon-cli
   ```
...
Run with no arguments to open a REPL (prompt `fastcon>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `ping`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/fastcon-cli`. |
...
(`./fastcon-cli <expr>`) and interactively (the REPL), and expressions compose the
