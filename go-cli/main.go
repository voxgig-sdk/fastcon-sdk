// Command Fastcon-cli is a CLI + REPL for the Fastcon SDK,
...
const prompt = "fastcon"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "ping proxy"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      FASTCON_APIKEY (api key), FASTCON_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: FASTCON_APIKEY carries the API key and
	// FASTCON_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("FASTCON_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("FASTCON_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewFastconSDK(opts)
