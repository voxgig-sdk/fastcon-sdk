...
[MCP](https://modelcontextprotocol.io) server exposing the Fastcon SDK as
two agent tools — `fastcon_list` and `fastcon_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/fastcon-mcp)
make build

# 2. Provide credentials via the environment
export FASTCON_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user fastcon \
  -- /absolute/path/to/fastcon-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./fastcon-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// fastcon_list: first page of records
{ "entity": "proxy" }
{ "entity": "proxy", "query": { } }

// fastcon_load: one record by id
{ "entity": "ping", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/fastcon-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user fastcon \
     -- "$PWD"/dist/*/fastcon-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `fastcon_list` and `fastcon_load` tools now appear
   in new sessions. Ask the agent to *"list proxy using fastcon"*
   and it calls `fastcon_list` with `{"entity":"proxy"}`.
...
```sh
export FASTCON_APIKEY=sk_live_xxx            # API key
export FASTCON_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `fastcon_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `fastcon_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FASTCON_APIKEY` | API key sent with every request. |
| `FASTCON_BASE` | Optional override of the API base URL. |
...
```sh
./fastcon-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"fastcon_load","arguments":{"entity":"ping","query":{"id":1}}}}'
```
