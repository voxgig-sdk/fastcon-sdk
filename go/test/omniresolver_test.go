...
import (
	...
	omni "github.com/voxgig-sdk/fastcon-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FastconError); is {
				out := map[string]any{
					"name":    "FastconError",
...
func omniCtx(arg any, client *sdk.FastconSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FastconSDK); is {
			client = live
