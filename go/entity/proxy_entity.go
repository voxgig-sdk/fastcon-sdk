...
func NewProxyEntity(client *core.FastconSDK, entopts map[string]any) *ProxyEntity {
	if entopts == nil {
		entopts = map[string]any{}
	}
...
func (e *ProxyEntity) GetName() string { return e.name }
...
func (e *ProxyEntity) MarkDeleted() {
	e.deleted = true
}


// Deleted reports whether a successful Remove has resolved on this instance.
func (e *ProxyEntity) Deleted() bool {
	return e.deleted
}


func (e *ProxyEntity) Make() core.Entity {
	opts := map[string]any{}
	for k, v := range e.entopts {
		opts[k] = v
	}
	return NewProxyEntity(e.client, opts)
}

func (e *ProxyEntity) Data(args ...any) any {
...
func (e *ProxyEntity) Match(args ...any) any {
	if len(args) > 0 && args[0] != nil {
...
// argument it returns the current data as an Proxy; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *ProxyEntity) DataTyped(data ...Proxy) Proxy {
	if len(data) > 0 {
		return typedFrom[Proxy](e.Data(asMap(data[0])))
	}
	return typedFrom[Proxy](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through Proxy (all fields
// optional at the wire level).
func (e *ProxyEntity) MatchTyped(match ...Proxy) Proxy {
	if len(match) > 0 {
		return typedFrom[Proxy](e.Match(asMap(match[0])))
	}
	return typedFrom[Proxy](e.Match())
}
...
func (e *ProxyEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
func (e *ProxyEntity) Load(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("load", e.name)
}



func (e *ProxyEntity) List(reqmatch map[string]any, ctrl map[string]any) (any, error) {
	utility := e.utility
...
// ProxyListMatch and returns []Proxy. It delegates to the untyped
// List (identical runtime) and converts at the typed boundary.
func (e *ProxyEntity) ListTyped(reqmatch ProxyListMatch, ctrl map[string]any) ([]Proxy, error) {
	res, err := e.List(asMap(reqmatch), ctrl)
	if err != nil {
		return nil, err
	}
	return typedSliceFrom[Proxy](res), nil
}



func (e *ProxyEntity) Create(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("create", e.name)
}


func (e *ProxyEntity) Update(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("update", e.name)
}


func (e *ProxyEntity) Remove(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("remove", e.name)
}


func (e *ProxyEntity) runOp(ctx *core.Context, postDone func()) (any, error) {
	utility := e.utility
