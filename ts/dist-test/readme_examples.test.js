...
const TEST_SEED = { "entity": { "ping": { "test01": { "id": "test01" } }, "proxy": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+FastconSDK\b/.test(code) ||
        /\bFastconSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+FastconSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/FastconSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
