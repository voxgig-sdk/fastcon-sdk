...
class ProxyEntity
  ...
  def make
    opts = @_entopts.dup
    ProxyEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = FastconHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Proxy, Hash] the current Proxy data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = FastconHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Proxy fields)
  def match_get
  ...
  #
  # @param reqmatch [ProxyListMatch, Hash, nil] match filter (any subset of
  #   Proxy fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Proxy>, Array] the matching Proxy items; raises FastconError on failure
