...
class PingEntity
  ...
  # @return [Ping, Hash] the current Ping data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Ping fields)
  def match_get
  ...
  # Create a new Ping.
  #
  # @param reqdata [PingCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Ping, Hash] the created Ping; raises FastconError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = FastconHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
