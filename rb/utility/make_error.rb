...
module FastconUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = FastconContext.new({}, nil)
    end
    op = ctx.op || FastconOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || FastconResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(FastconError) ? err.msg : err.to_s
    ...
    sdk_err = FastconError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(FastconError)
