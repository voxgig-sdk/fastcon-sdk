...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = FastconOmni.make_runner(TEST_JSON_FILE, FastconSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : FastconOmni.errify(err)
    end
