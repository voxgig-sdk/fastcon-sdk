...
module FastconUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = FastconResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FastconResponse.new(fetched)
    else
