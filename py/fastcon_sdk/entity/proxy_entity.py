...
from fastcon_sdk.fastcon_types import (
    Proxy,
    ProxyListMatch,
)
...
class ProxyEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ProxyEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Proxy]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Proxy().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
