# Fastcon SDK test runner

from __future__ import annotations
import os
import json

from fastcon_sdk.utility.voxgig_struct import voxgig_struct as vs


class FastconTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FastconTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FastconTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FastconTestRunner.getenv("FASTCON_TEST_LIVE")
        override = FastconTestRunner.getenv("FASTCON_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FastconTestRunner.getenv(key)
        ...
        explain = FastconTestRunner.getenv("FASTCON_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["FASTCON_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if FastconTestRunner._test_control is not None:
            return FastconTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FastconTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FastconTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FastconTestRunner.load_test_control()
...
def load_env_local():
    FastconTestRunner.load_env_local()


def env_override(m):
    return FastconTestRunner.env_override(m)


def entity_data(v):
    return FastconTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FastconTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FastconTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FastconTestRunner.load_test_control()


def live_client_options():
    return FastconTestRunner.live_client_options()


def live_delay_ms():
    return FastconTestRunner.live_delay_ms()
